...
├── config/                 # Starlark configuration files
│   ├── autor.star
│   ├── client.star
│   └── presence.star
├── core/                   # Core logic and entities
├── data/
│   └── ide/                # JSON mapping files for 32‑bit and 64‑bit IDEs
│       ├── ide32bit.json
│       └── ide64bit.json
├── src/                    # Source code
├── LICENSE
...
You need a Discord application with Rich Presence enabled. The client ID and public key are stored in `config/client.star`:
...
The file `config/presence.star` defines which processes to watch, how to parse window titles, and what to display in Discord. Each application entry has the following fields:

* `id` — internal identifier.
* `name` — human‑readable IDE name.
* `processes` — list of executable names (case‑insensitive).
* `title_rules` — list of regex patterns with priorities to extract the project name.
* `presence` — template for Discord status fields (`details`, `state`, `large_image`, `large_text`).

### IDE mappings

The JSON files in `data/ide/` define which executable names correspond to which IDE display names:
...
Once the daemon is running, it automatically detects supported JetBrains IDEs and updates your Discord status. No further action is required. The status clears when the IDE is closed.
